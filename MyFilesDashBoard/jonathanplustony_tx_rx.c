...
void CAN_ReceiveMsg(uint8 rxMailbox) 
{
    ...
    #endif /* CY_PSOC3 || CY_PSOC5 */
        {
			...
			can_queue[can_tail].id = CAN_GET_RX_ID(rxMailbox);
			can_queue[can_tail].millicounter = 0;

			for(rx_index = 0; rx_index < rx_length; rx_index++)
			{
				can_queue[can_tail].data[rx_index] =
					CAN_RX_DATA_BYTE(rxMailbox, rx_index);
			} // for all bytes, copy data to can_queue
...
`#start MESSAGE_temp_TRASMITTED` -- section removed from template
						uint8_t i;
